...
export type SymbolInfo = { symbol: string; baseAsset: string; quoteAsset: string; volume: number; volumeSpike: number; change: number; lastPrice: number };
...
export function useBinanceMarket(symbol: string, interval: string, paused: boolean) {
  ...
  useEffect(() => {
    ...
    const loadSeed = async () => {
      ...
      try {
        const [klinesResponse, tickerResponse, depthResponse] = await Promise.all([
           fetch(`${REST}/fapi/v1/klines?symbol=${symbol}&interval=${interval}&limit=1000`),
    ...
    void loadSeed();
    const socket = new WebSocket(`${WS}${lower}@aggTrade/${lower}@depth@100ms/${lower}@kline_${interval}/${lower}@forceOrder`);
  ...
  useEffect(() => {
    Promise.all([fetch(`${REST}/fapi/v1/exchangeInfo`), fetch(`${REST}/fapi/v1/ticker/24hr`)]).then(async ([exchangeResponse, tickerResponse]) => {
      ...
      const tickers = new Map((tickerPayload as Array<Record<string, unknown>>).map((item) => [String(item.symbol), item]));
       const rawList = (payload.symbols as Array<Record<string, unknown>>).filter((item) => item.quoteAsset === 'USDT' && item.contractType === 'PERPETUAL' && item.status === 'TRADING').map((item) => {
        const ticker = tickers.get(String(item.symbol));
         return { symbol: String(item.symbol), baseAsset: String(item.baseAsset), quoteAsset: 'USDT', volume: num(ticker?.quoteVolume), volumeSpike: 1, change: num(ticker?.priceChangePercent), lastPrice: num(ticker?.lastPrice) };
       });
...
export function useMarketPulse(symbol: string) {
    ...
    const loadPulse = async () => {
      try {
        const [interestResponse, premiumResponse] = await Promise.all([
          fetch(`${REST}/fapi/v1/openInterest?symbol=${symbol}`),
          fetch(`${REST}/fapi/v1/premiumIndex?symbol=${symbol}`),
        ]);
